...
import { refreshCookie } from '@render/NeteastApi/auth'
import { dailySignin } from '@render/NeteastApi/user'
...
export function dailyTask() {
    ...
    if (isAccountLoggedIn() && (lastDate === undefined || lastDate !== dayjs().date())) {
        console.debug('[debug][common.js] execute dailyTask')
        refreshCookie().then(() => {
            console.debug('[debug][common.js] 刷新cookie')
            store.commit('data/updateData', {
...
export function splitAlbumTitle(title) {
    let keywords = ['Bonus Tracks Edition', 'Complete Edition', 'Deluxe Edition', 'Deluxe Version', 'Tour Edition']
    for (let keyword of keywords) {
        if (title.includes(keyword) === false) {
            continue
        }
