...
const db = new Dexie('EstherMusic')
...
db.version(3)
    ...
    .upgrade((tx) =>
        tx
            .table('trackSources')
            .toCollection()
            .modify((track) => !track.createTime && (track.createTime = new Date().getTime())),
    )
...
export function cacheTrackSource(trackInfo, url, bitRate, from = 'netease') {
    ...
    let artist
    if (trackInfo.ar[0]?.name) {
        artist = trackInfo.ar[0]?.name
    } else if (trackInfo.artists) {
        artist = trackInfo.artists[0]?.name
    } else {
        artist = trackInfo.pc.ar
    }
